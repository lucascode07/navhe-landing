...
const FormPartner = ({ sendType }: Props) => {
  ...
  return (
    <div className="main-container o-form-partners">
      <div className="form-header">
        <h1>Formulario de Asociados</h1>
        <p>
          Por favor complete todos los espacios requeridos para tener presente
          su información y asi poder ubicarlo dentro del horario disponible y
          dentro del perímetro cercano de su domicilio.
        </p>
      </div>
      {showResponseMessage ? (
        <ResponseMessage />
    ...
    `}
              ...
              {errors.address && (
                <small>{errors.address.message as string}</small>
              )}
            </div>
    ...
    `}
          ...
          <div className="form-body__group">
            <div className='form-group'>
              <input className={`
                        ${watch('business') ? 'active' : ''} 
                        ${(!errors.business && touchedFields.business) ? 'valid' : ''}
                        ${errors.business ? 'invalid' : ''}
                      `} id='business' type="text" {...register('business', {
                required: {
                  value: true,
                  message: 'Campo requerido'
                },
              ...
              {
                errors.business && <small>{errors.business.message as string}</small>
              }
            </div>
            <div className="subgroup">
              <div className='form-group'>
                <select className={`
                          ...
                          ${(!errors.category && touchedFields.category) ? 'valid' : ''}
                  ...
                  <option value=""></option>
                  <option value="Mañana">Comidas</option>
                  <option value="Noche">Bebidas</option>
                </select>
                <label htmlFor='category'>Categoría</label>
                {
                  errors.category && <small>{errors.category.message as string}</small>
                }
              </div>
              <div className='form-group'>
                <select className={`
                          ${watch('type') ? 'active' : ''} 
                          ${(!errors.type && touchedFields.type) ? 'valid' : ''}
                          ${errors.type ? 'invalid' : ''}
                  ...
                  <option value=""></option>
                  <option value="Mañana">Pollería</option>
                  <option value="Noche">Cevichería</option>
                </select>
                <label htmlFor='type'>Tipo</label>
                {
                  errors.type && <small>{errors.type.message as string}</small>
                }
              </div>
            </div>
            <div className='form-group'>
